...
public class MySqlDAOFactory {
	...
	// declaramos un objeto privado y estatico de la clase que implementa el patron
	// singleton
	...
	private MySqlDAOFactory() {
		try {
			...
			conexion = DriverManager.getConnection("jdbc:mysql://localhost/tema3", "root","temporal");
	...
	public ParadasDAOImpl getParadas() {
		return new ParadasDAOImpl(conexion);
	}
	
	public LineaParadaDAOImpl getLineaParada(){
		return new LineaParadaDAOImpl(conexion);
	}

	public LineaDAOImpl getLineas() {
		return new LineaDAOImpl(conexion);
	}
